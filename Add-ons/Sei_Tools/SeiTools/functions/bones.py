...
#=== Assign/Remove rig type.

def find_bone_layer(bone): # Used in rig_type_settings().
...
def bone_assign_rig_type(context):
    sei_vars = context.scene.sei_variables
    rig_type = sei_vars.rig_types
    if rig_type == ' ':
        rig_type = None
...
def bone_remove_rig_type(context):
    for bone in context.selected_pose_bones:
        context.object.pose.bones[bone.name],rigify_type = ''
